//JavaScript for The Sport Shop Website
...
//This function should update confirmation.html with all the details of the payment made
function finish(){
	var errInput=0;
	//document.cookie="CCOK=; expires=Thu, 01 Jan 1970 00:00:00 UTC";
	
	if($('#v').is(":checked")){										//This IF statement allows to know which payment method has been selected 
	}
	else if($('#m').is(":checked")){
	}
	else if($('#vd').is(":checked")){
	}
	...
	if($("#cn").val()=="" && isNaN($("#cn").val())){			//This IF statement allows to check if the field card number has been filled with

		alert("Please input a valid card number.");
		errInput++;
	}	
	
	if($("#cvv").val()=="" && isNaN($("#cvv").val())){	//This IF statement allows to check if the field CVV has been filled with 							//a number and update confirmation.html
	
		alert("Please input a valid cvv number.");
		errInput++;
	}
	
	if($("#fn").val()!="" && isNaN($("#fn").val())){		//This IF statement allows to check if both fields first name and last name have been filled with 
		var fn=($("#fn").val());							//a string, concatenate both value to form full name and update confirmation.html
		
		if($("#ln").val()!="" && isNaN($("#ln").val())){
			var ln=($("#ln").val());
		}
		else{
		...
		if($("#fn").val()!="" && isNaN($("#fn").val()) && $("#ln").val()!="" && isNaN($("#ln").val())){
			$("p7").text("The name of the cardholder is: " + fn + " " + ln);
		}
	}
	else{
	...
	if(errInput===0){
		//This function check the validity of the credit card number
		$(function(){
			$isValid=false;		//Setting default value
			$("#cn").validateCreditCard(function(result){
				result.valid? $isValid=true : isValid=false; //Showing the 2 posible outcomes and setting up for the action to be taken in function of those outcomes
			});
			
			$("#cn").ready(function(isValid){
				
				$isValid? document.cookie="CCOK=ok;time() +60)"						// if the CC number is valide go to confirmation.hmtl  document.cookie="CCOK=;time() +60)
				:alert("The credit card number you have input is not valid, please try again.");				//else ask for the user for a new input
			});
		});
		
		
			var year=(new Date()).getFullYear();	//get the year
			var text = "";							// initializing the text variable

			var possible = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"; //array of characters
			
			//randomly picking 3 characters
			for( var i=0; i < 3; i++ ){
				text += possible.charAt(Math.floor(Math.random() * possible.length));
			}
			
			var num= (Math.random()*100000000).toFixed(0);	// generating a random number
			
			var toDisplay=  year + "-" + text + "-" +num;	//concateneting the result to get the order number
			
			document.cookie="orderID="+toDisplay+";time()+3600";	//genearting the cookie orderID
		
			var add=$("#add").val();		//getting the content of the address box in checkOut.html
			
			document.cookie="Address="+add+";time()+3600";	//Generating the cookie Address
...
(function( $ ) {
	...
	$.Shop.prototype = {
		init: function() {
			...
			this.$checkoutOrderForm = this.$element.find( "#checkout-order-form" ); // Checkout user details form DNA
			this.$shipping = this.$element.find( "#sshipping" ); // Element that displays the shipping rates DNA
		...
		displayCart: function() {
			if( this.$formCart.length ) {
				...
				for( var i = 0; i < items.length; ++i ) {
					...
					var qty = item.qty;
					var html = "<tr><td class='pname"+[i]+"'>" + product + "</td>" + "<td class='pqty"+[i]+"'><input type='text' value='" + qty + "' class='qty"+[i]+"'/></td>" + "<td class='pprice"+[i]+"'>" + price + "</td><td class='remove' id='"+[i]+"'></tr>";
				...
				for( var j = 0; j < cartItems.length; ++j ) {
					...
					var cartHTML = "<tr><td class='pname"+[j]+"'>" + cartProduct + "</td>" + "<td class='pqty"+[j]+"'>" + cartQty + "</td>" + "<td class='pprice"+[j]+"'>" + cartPrice + "</td><td class='remove' id='"+[j]+"'></tr>";
					
					$cartBody.html( $cartBody.html() + cartHTML );console.log(cartItems);
				}
...
//This function empty the session storage when the finish button is click inconfirmation.html
